...
#if defined(FPGA)
                card->busClock_Hz = HOST_SET_CARD_CLOCK(card->host.base, card->host.sourceClock_Hz, card->host.sourceClock_Hz);
#else
        ...
        switch (card->currentTiming)
        {
            ...
            case kSD_TimingDDR50Mode:
                ...
                if (error == kStatus_Success)
                {
                    ...
                    card->busClock_Hz =
#if defined(FPGA)
                        HOST_SET_CARD_CLOCK(card->host.base, card->host.sourceClock_Hz, card->host.sourceClock_Hz);
#else
