...
#if defined(DMA_ADDRESS_OFFSET) && DMA_ADDRESS_OFFSET
#define ADDRESS(x) (void *)((uint32_t)x + DMA_ADDRESS_OFFSET)
#else
#define ADDRESS(x) (x)
#endif
...
int main(void)
{
    ...
    /* Start to echo. */
    sendXfer.data = ADDRESS(g_txBuffer);
    sendXfer.dataSize = ECHO_BUFFER_LENGTH;
    receiveXfer.data = ADDRESS(g_rxBuffer);
