...
void BOARD_I2C_ReleaseBus(void)
{
    ...
    for (i = 0; i < 9; i++)
    {
        GPIO_WritePinOutput(I2C_RELEASE_SCL_PORT, I2C_RELEASE_SCL_PIN, 0U);
        i2c_release_bus_delay();
...
int main(void)
{
    ...
    if (true == isThereAccel)
    {
        ...
        for (i = 0; i < ACCEL_READ_TIMES; i++)
        {
            ...
            {
                reTrans = LPI2C_ReadAccelRegs(BOARD_ACCEL_I2C_BASEADDR, g_accel_addr_found, ACCEL_STATUS, &status0_value, 1);
                if (reTrans == false )
                {
                    return -1;
                }
