...
int main(void)
{
    ...
    INTMUX_Init(INTMUX0);
    INTMUX_EnableInterrupt(INTMUX0, 0, LPTMR0_IRQn);
...
void LPTMRx_IRQHandler(void)
{
    ...
    while (kSPM_DcdcStableOKFlag != (kSPM_DcdcStableOKFlag & SPM_GetDcdcStatusFlags(SPM)))
    {
    }
...
void DCDCx_Configuration(void)
{
    ...
    SPM_SetDcdcVdd1p2ValueBuck(SPM, 0x6); /* Trim the 1P2 to output 1.225V */
    ...
    mSpmDcdcIntegratorConfigStruct.vdd1p2Value = 1.225f;
    ...
    SPM_EnableRegulatorInRunMdoe(SPM, true, kSPM_DcdcLdo);               /* Enable the DCDC. */
    SPM_EnableRegulatorInRunMdoe(SPM, false, kSPM_CoreLdo | kSPM_RfLdo); /* Disable the Core LDO and RF LDO. */
