...
Board settings
==============
The default jumper settings should be used with the exception of the following:
J53: jumper should be populated on pins 2-3.
J47: open (note that the shorting trace on the bottom of the board should be cut).

Prepare the demo
1.  Connect the J-link debugger between the PC and the board's debug port.
2.  Connect the USB-RS232 converter between the PC and the board's UART-RS232 port.
3.  Open a serial terminal on PC for board's UART-RS232 with these settings:
    ...
    - One stop bit
    - No flow control
4.  Set the SoC booting up from core 0 (CM4 core), since the program would be downloaded and run in core 0.
5.  Build the core 1's image first and build the core 0's image with core 1's image linked inside.
6.  Download the program to the target board.
7.  Press the reset button on your board to begin running the demo.
8.  Don't debug dual core at the same time, because cm0plus is in VLLS0 and could not be
    connected.
